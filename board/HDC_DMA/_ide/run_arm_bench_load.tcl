# Load ARM bench ELF on already-programmed board; poll @ 0x00100400.
...
if {![file exists $ARM_BENCH_ELF]} {
    error "Missing ARM bench ELF: $ARM_BENCH_ELF"
}

puts "Loading ARM bench ELF (PL must already be programmed):"
puts "  app: $ARM_BENCH_ELF"

connect -url $HW_URL
wait_targets 20
load_elf_on_a9_0 $ARM_BENCH_ELF "arm_bench"
con

puts "CPU running ARM bench — polling @ [format 0x%08X $ARM_BENCH_BASE] (max ${MAX_WAIT_SEC}s)..."
...
if {!$done} {
    error "Timeout waiting for ARM bench"
}
