# Program narrow DMA bench ELF, poll results @ 0x00100000.

set SCRIPT_DIR [file dirname [file normalize [info script]]]
source [file join $SCRIPT_DIR paths.tcl]
set BENCH_ELF [file join $HDC_DMA_ROOT app build Final_HDC_dma_bench_narrow.elf]
...
foreach f [list $BITFILE $PS7_INIT $FSBL_ELF $BENCH_ELF] {
    if {![file exists $f]} {
        error "Missing required file: $f"
    }
}

puts "Programming HDC_DMA narrow bench with:"
puts "  bitstream: $BITFILE"
puts "  app:       $BENCH_ELF"

connect -url $HW_URL
program_zed_board $BITFILE $PS7_INIT $FSBL_ELF $BENCH_ELF $VIVADO_PL 1

puts "CPU running narrow bench — polling @ [format 0x%08X $BENCH_BASE] (max ${MAX_WAIT_SEC}s)..."
