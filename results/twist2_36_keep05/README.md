...
Generated: 2026-07-14T06:11:38Z
...
## Mask audit (why 0.00 pp gap is authentic)

Full sweep: local and pooled masks are built independently; gap is 0 because pruning is
**lossless** on all held-out TEST subjects (local = pooled = unpruned). Masks still differ
in bit pattern — fast audit:

```bash
python3 scripts/twist2_mask_audit_fast.py
```

See [`mask_audit_fast.json`](mask_audit_fast.json) (e.g. S19 @ keep=0.5: Jaccard 0.70, not identical).
